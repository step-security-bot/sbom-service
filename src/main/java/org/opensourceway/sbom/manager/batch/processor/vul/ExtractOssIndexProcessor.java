...
public class ExtractOssIndexProcessor implements ItemProcessor<List<ExternalPurlRef>, Set<Pair<ExternalPurlRef, Object>>>, StepExecutionListener {

    private static final Logger logger = LoggerFactory.getLogger(ExtractOssIndexProcessor.class);
    ...
    private VulService ossIndexService;
    ...
    public Set<Pair<ExternalPurlRef, Object>> process(List<ExternalPurlRef> chunk) {
        ...
        logger.info("start ExtractOssIndexProcessor sbomId:{}, chunk size:{}", sbomId, chunk.size());

        Set<Pair<ExternalPurlRef, Object>> resultSet = ossIndexService.extractVulForPurlRefChunk(sbomId, chunk);
        logger.info("finish ExtractOssIndexProcessor resultSet size:{}", resultSet.size());
        return resultSet;
    }
